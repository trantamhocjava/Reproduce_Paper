...
class ModelXtoC(PretrainedResNetModel):
    def __init__(self, cfg):
        ...
        if self.use_input_conv_layers:
            ...
            previous_layer_dims = sum(
                [self.conv_layer_dims[layer] for layer in self.input_conv_layers]
            )
    ...
    def loss(self, outputs, data_dict):
        ...
        C_hat = outputs["C"]
        assert C_hat.shape[1] == len(self.C_cols), print(
            "C_hat: %s, len C_cols: %s" % (C_hat.shape, len(self.C_cols))
        )
    ...
    def analyse_predictions(self, y_true, y_pred, info={}):
        ...
        metrics_C = analysis.assess_performance(
            y=C,
            yhat=C_hat,
            names=self.C_cols,
    ...
    def load_pretrained(self):
        ...
        for name, param in pretrained_state.items():
            ...
            if not name.startswith(prefix) or any(
                [name.startswith(var) for var in self.pretrained_exclude_vars]
            ):
...
class ModelOracleCtoY(object):
    def __init__(self, model_type="lr", new_kwargs={}):
        if model_type == "lr":
            ...
            model = SVR
            model_kwargs = {"C": 5.0, "kernel": "rbf", "gamma": "scale"}
            if new_kwargs:
    ...
    def analyse_predictions(self, y_true, y_pred):
        assert y_true.shape == y_pred.shape, print(
            "y_true: %s, y_pred: %s" % (y_true.shape, y_pred.shape)
        )
...
class ModelXtoChat_ChatToY(InterventionModelOnC, PretrainedResNetModel):
    ...
    def get_data_dict_from_dataloader(self, data):
        ...
        inputs = {"image": X}
        labels = {"y": y, "C_feats": C_feats, "C_feats_not_nan": C_feats_not_nan}
    ...
    def loss(self, outputs, data_dict):
        ...
        # Parse outputs from deep model
        y_hat = outputs["y"]
        assert y_hat.shape[1] == len(self.y_cols), print(
            "y_hat: %s, len y_cols: %s" % (y_hat.shape, len(self.y_cols))
        )
    ...
    def analyse_predictions(self, y_true, y_pred, info={}):
        ...
        metrics_y = analysis.assess_performance(
            y=y,
            yhat=y_hat,
            names=self.y_cols,
            prediction_type="continuous_ordinal",
        ...
        print(
            "%s epoch loss for %s: %2.6f; RMSE %2.6f; correlation %2.6f (n=%i)"
            % (
                phase,
                str(self.y_cols),
                epoch_loss,
                metrics_y["%s_xrkl_rmse" % phase],
                metrics_y["%s_xrkl_r" % phase],
                len(y_hat),
            )
        )
        metrics_all["%s_epoch_loss" % phase] = epoch_loss
...
class ModelXtoCY(PretrainedResNetModel):
    ...
    def loss(self, outputs, data_dict):
        ...
        # Final loss
        loss = loss_y + loss_C
        loss /= sum(self.additional_loss_weighting) + 1.0
        ...
        loss_details = {"loss_ratio": loss_ratio}
        return loss, loss_details
    ...
    def analyse_predictions(self, y_true, y_pred, info={}):
        ...
        print(
            "Loss_y divided by loss_C is %2.3f (median ratio across batches)"
            % np.median(loss_ratios)
        )
        ...
        if self.y_loss_type == "cls":
            ...
            for i, N_cls in enumerate(self.classes_per_y_col):
                y_hat_new.append(
                    np.argmax(y_hat[:, start_id : start_id + N_cls], axis=1)
                )
                start_id += N_cls
        ...
        if self.C_loss_type == "cls":
            ...
            for i, N_cls in enumerate(self.classes_per_C_col):
                C_hat_new.append(
                    np.argmax(C_hat[:, start_id : start_id + N_cls], axis=1)
                )
                start_id += N_cls
                ...
                if self.test_time_intervention_method == "random":
                    if self.C_loss_type == "reg":
                        rand_ids = np.array(
                            [
                                np.random.choice(
                                    range(N_concepts), N_given, replace=False
                                )
                                for _ in range(B)
                            ]
                        )  # (B,N_given)
