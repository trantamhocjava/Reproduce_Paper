...
class EBM_GL(nn.Module):
    ...
    def __init__(
        self, args, num_classes, input_size=1000, hid_size=1000, cpt_size=None
    ):
...
class ECBM(nn.Module):
    def __init__(self, args, emb_size, cpt_size, hid_size, n_classes):
        ...
        if args.freezebb == True:
            for param in list(
                set(self.parameters()) - set(self.energy_model.parameters())
            ):
    ...
    def load_dict(self, path):
        ...
        state_dict = {
            k: v for k, v in weights_dict.items() if k in model_state_dict.keys()
        }
        # print(state_dict)
