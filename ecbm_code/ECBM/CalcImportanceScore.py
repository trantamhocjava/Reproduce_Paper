...
for check_idx, y in enumerate(y_labelset):
    ...
    print(
        "=" * 5,
        f"Joint Importance Score for Label {int(y)}: {classes[int(y)]}",
        "=" * 5,
    )
    ...
    print("=" * 5, "Most important joint concept set", "=" * 5)
    print(list(c_pred_jointset[much_likely].astype(int)))
    most_import_joint_c.append(list(c_pred_jointset[much_likely].astype(int)))
    print(
        "=" * 5,
        f"Marginal Importance Score for Label {int(y)}: {classes[int(y)]}",
        "=" * 5,
    )
    print(marginal_importance[check_idx])
    print(
        "=" * 5, f"Concept groundtruth for Label {int(y)}: {classes[int(y)]}", "=" * 5
    )
