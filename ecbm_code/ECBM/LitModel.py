...
from metrics import AverageMeter, MetricComputer
...
class LitModel(pl.LightningModule):
    ...
    def __init__(self, logdir, args):
        ...
        logging.basicConfig(
            filename=logdir + "/log.txt", format="%(message)s", level=logging.INFO
        )
        logging.info(args)
    ...
    def training_step(self, batch, batch_idx):
        ...
        c_overall_acc = sklearn.metrics.accuracy_score(
            batch_C.cpu().numpy(), met_c.cpu().numpy()
        )
        ...
        self.log(
            "accuracy_train_c_overall", c_overall_acc, on_epoch=True, on_step=False
        )
        self.log(
            "accuracy_train_c", metrics_c["accuracy"], on_epoch=True, on_step=False
        )
        self.log(
            "accuracy_train_y_xy", metrics_xy["accuracy"], on_epoch=True, on_step=False
        )
        self.log(
            "accuracy_train_y_cy", metrics_cy["accuracy"], on_epoch=True, on_step=False
        )
        return loss
    ...
    def validation_step(self, batch, batch_idx):
        ...
        self.log(
            "accuracy_val_y_xy", metrics_xy["accuracy"], on_epoch=True, on_step=False
        )
        self.log(
            "accuracy_val_y_cy", metrics_cy["accuracy"], on_epoch=True, on_step=False
        )
        return loss
    ...
    def test_step(self, batch, batch_idx):
        ...
        _, met_c = torch.min(out_c, 2)
        metrics_c = self.computer(met_c, batch_C)
        metrics_c_overall = sklearn.metrics.accuracy_score(
            batch_C.cpu().numpy(), met_c.cpu().numpy()
        )

        self.log(
            "accuracy_test_c_overall", metrics_c_overall, on_epoch=True, on_step=False
        )
        self.log("accuracy_test_c", metrics_c["accuracy"], on_epoch=True, on_step=False)
        self.log(
            "accuracy_test_y_xy", metrics_xy["accuracy"], on_epoch=True, on_step=False
        )
        self.log(
            "accuracy_test_y_cy", metrics_cy["accuracy"], on_epoch=True, on_step=False
        )
        return [
            metrics_c_overall,
            metrics_c["accuracy"],
