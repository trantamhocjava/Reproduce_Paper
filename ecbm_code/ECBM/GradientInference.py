    ...
    def inference(self, x, y, c):
        ...
        if self.intervene_type == "group":
            ...
            gt_in_idx = torch.tensor(
                select_concept_group(gt_in, len_group_concept)
            ).cuda()
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--intervene_type", type=str, choices=["group", "individual"], default="None"
    )
    ...
    if not os.path.exists(exp_dir):
        os.mkdir(os.path.join(args.exp_dir, f"{uuid}-{args.dataset}-{args.backbone}"))

    logging.basicConfig(
        filename=exp_dir + "/log.txt", format="%(message)s", level=logging.INFO
    )

    logging.info(args)

    network = ECBM(
        args, args.emb_size, args.cpt_size, args.hid_size, n_classes=args.n_classes
    )
    ...
    for idx, i in enumerate(test_loader):
        ...
        energy_after_intv, energy_before = inferer.inference(x, y, c)
        ...
        if energy_after_intv is not None:
            ...
            print(f"y_acc: {y_acc}, c_acc_overall: {c_acc_overall}, c_acc: {c_acc}")
