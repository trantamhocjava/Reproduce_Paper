...
class AnimalDataset(Dataset):
    def __init__(self, data_path, transform):
        ...
        predicate_binary_mat = np.array(
            np.genfromtxt(f"{root_dir}/predicate-matrix-binary.txt", dtype="int")
        )
...
def load_data(
    data_path, batch_size, shuffle=True, num_workers=4, resol=224, is_training=True
):
    if is_training:
