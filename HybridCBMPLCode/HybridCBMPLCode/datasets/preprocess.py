...
def load_classes():
    classes = []
    for data in ['HAM10000']:
        cls = list(pd.read_csv(f'./{data}/class.csv')['class'].unique())
        classes.extend(cls)
    return classes

def load_assertions(file_path):
    save_path = '/kaggle/working/assertions-en.csv'
    
    if os.path.exists(save_path):
        print(f"Đã tìm thấy file cache tại {save_path}, đang tải...")
        return pd.read_csv(save_path)
        
    print(f"Đang đọc dữ liệu gốc từ: {file_path}")
    
    # Đọc trực tiếp từ file assertions.csv (ngăn cách bởi dấu Tab)
    df = pd.read_csv(file_path, sep='\t', names=['uri', 'relation', 'start', 'end', 'info'],
                     usecols=['relation', 'start', 'end', 'info'], dtype=str)
                     
    # --- Phần logic lọc dữ liệu tiếng Anh (giữ nguyên của tác giả) ---
    ...
    # Lưu kết quả sang thư mục working để dùng cho lần sau
    df.to_csv(save_path, index=False)
    return df

def collect_conceptnet_concepts(conceptnet_csv_path):
    translator_path = '/kaggle/working/ConceptTranslator' 
    
    # Gọi hàm xử lý dữ liệu
    raw_data = load_assertions(conceptnet_csv_path)
...
def collect_generated_concepts():
    HAM10000 = json.load(open('/kaggle/input/datasets/loinguyen57/data-for-translator/class2concepts.json', 'r'))
...
def tokenize(context_length: int = 77):
    ...
    conceptNet = pd.read_json('/kaggle/working/ConceptTranslator/conceptNet.json')
    generated = pd.read_json('/kaggle/working/ConceptTranslator/generatedConcepts.json')
    concepts = pd.concat([conceptNet, generated]).reset_index(drop=True)
    concepts.to_json('/kaggle/working/ConceptTranslator/conceptsBank.json', index=False)
    ...
    if not os.path.exists('/kaggle/working/ConceptTranslator/conceptsBank_tokens.pt'):
        torch.save(result, '/kaggle/working/ConceptTranslator/conceptsBank_tokens.pt')
...
@torch.no_grad()
def concept(clip_model_type='ViT-L/14', device=0):
    ...
    loader = DataLoader(_Dataset(tokens), batch_size=512, shuffle=False, num_workers=4)
    all_embeddings = []
    all_tokens = []
...
@torch.no_grad()
def coco(clip_model_type='ViT-L/14', device=0):  # 566747
    device = torch.device(device)
    clip_model, preprocess = clip.load(clip_model_type, device=device, jit=False)
    clip_model_name = clip_model_type.replace('/', '_')
    out_path = f"/kaggle/working/COCO_{clip_model_name}_train.pkl"
    json_path = '/kaggle/working/coco_data.json'
    if os.path.exists(out_path):
        print(f"File exists: {out_path}")
        return
    if os.path.exists(json_path):
        ...
        with open('/kaggle/input/datasets/loinguyen57/coco-translator/train_caption.json', 'r') as f:
        ...
        for d in tqdm(captions):
            img_id = d["image_id"]
            caption = d['caption']
            filename = f"/kaggle/input/datasets/loinguyen57/coco-translator/train2014/train2014/COCO_train2014_{int(img_id):012d}.jpg"
            if not os.path.isfile(filename):
                filename = f"/kaggle/input/datasets/loinguyen57/coco-translator/val2014/val2014/COCO_val2014_{int(img_id):012d}.jpg"
            if not os.path.isfile(filename):
                print(f"File not found: {filename}")
                continue
            data.append({"image_id": img_id, "image": filename, "caption": caption})
        data = pd.DataFrame(data)
        data.to_json('/kaggle/working/coco_data.json', index=False)
    ...
    loader = DataLoader(_Dataset(data.to_dict(orient='records'), preprocess), batch_size=128, shuffle=False,
                        num_workers=4)
    all_embeddings = []
    all_tokens = []
