...
def main(config):
    ...
    kltn_utils.rank_zero_info_newline("Load train, val dataset (img features)")
    train_loader, train_img_feat = utils.load_train(config, clip_model)
    val_loader = utils.load_val(config, clip_model, "val")
    kltn_utils.rank_zero_info_newline("train")
    utils.print_first_batch(train_loader)
    kltn_utils.rank_zero_info_newline("val")
    utils.print_first_batch(val_loader)

    kltn_utils.rank_zero_info_newline("Load hybrid concept bank")
    concept_bank = utils.load_concept_bank(config, train_img_feat)

    kltn_utils.rank_zero_info_newline("Load model")
    model = HybridCBMTrain(config=config, concept_bank=concept_bank)
    ...
    kltn_utils.rank_zero_info_newline("Result of best model on val")
    tester = Trainer(
        accelerator="gpu",
        devices=1,
        precision=32,
