...
class StaticConceptBank(BaseCB):
    def __init__(self,
        ...
        concept_feature_dir = self.exp_root.joinpath("concepts_feat")
    ...
    def select_concept(self, img_features, text_features, num_images_per_class):
        if not self.select_idx_save_path.exists():
            ...
            self._concepts.iloc[select_cid].reset_index(drop=True).to_csv(self.exp_root.joinpath('selected_concepts.csv'), index=False)
        else:
