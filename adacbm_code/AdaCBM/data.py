...
class DataModule(pl.LightningDataModule):
    ...
    def __init__(
        ...
        self.img_feat_save_dir = {
            mode: self.img_split_path.joinpath(
                "img_feat_{}_{}_{}{}_{}.pth".format(
                    mode,
                    n_shots,
                    int(self.use_img_norm),
                    int(self.use_txt_norm),
                    clip_model.replace("/", "-"),
                )
                if mode == "train"
                else "img_feat_{}_{}{}_{}.pth".format(
                    mode,
                    int(self.use_img_norm),
                    int(self.use_txt_norm),
                    clip_model.replace("/", "-"),
        ...
        self.label_save_dir = {
            mode: self.img_split_path.joinpath(
                "label_{}_{}.pth".format(mode, n_shots)
                if mode == "train"
                else "label_{}.pth".format(mode)
            )
            for mode in ["train", "val", "test"]
        }
        ...
        self.concept_pair_save_dir = self.data_root.joinpath(
            "concept_pair.pth"
        )  # concept pairs for selected concepts

        self.concepts_raw_save_dir = self.data_root.joinpath(
            "concepts_raw_selected.npy"
        )
        self.concept2cls_save_dir = self.data_root.joinpath("concept2cls_selected.npy")
        ...
        self.cls_names = np.load(
            cls_names_path
        ).tolist()  # for reference, the mapping between indices and names
        self.num_concept = num_concept

        # handling image related data
        self.splits = {
            split: utils.pickle_load(
                self.img_split_path.joinpath("class2images_{}.p".format(split))
            )
        ...
        self.prepare_img_feat(
            self.splits, self.n_shots, self.clip_model, self.clip_ckpt
        )
        ...
        if self.n_shots != "all":
            ...
            self.num_images_per_class = [
                len(images) for _, images in self.splits["train"].items()
            ]
        ...
        if self.is_concept2type:
            self.concept2type = np.load(
                self.concept2cls_path.replace("concept2cls.npy", "concept2type.npy")
            )
        ...
        if self.use_cls_name_init != "none":
            self.gen_init_weight_from_cls_name(
                self.cls_names, self.concepts_raw[self.select_idx]
            )

        if self.use_cls_sim_prior != "none":
            split = "train"
            self.gen_mask_from_img_sim(
                self.img_feat[split], self.n_shots, self.label[split][:: self.n_shots]
            )
    ...
    def gen_init_weight_from_cls_name(self, cls_names, concepts):
        ...
        cls_name_feat = utils.prepare_txt_feat(
            cls_names, clip_model_name=self.clip_model, ckpt_path=self.clip_ckpt
        )
        concept_feat = utils.prepare_txt_feat(
            concepts, clip_model_name=self.clip_model, ckpt_path=self.clip_ckpt
        )
    ...
    def select_concept(
        ...
        if not self.select_idx_save_dir.exists() or (
            self.force_compute and not clip_ckpt
        ):
            print("select concept")
            self.select_idx, selected_concept_pairs = concept_select_fn(
                img_feat_train,
                concept_feat,
                concept2cls,
                num_concepts,
            ...
            if selected_concept_pairs is not None:
                torch.save(
                    torch.tensor(selected_concept_pairs), self.concept_pair_save_dir
                )
    ...
    def prepare_txt_feat(self, concepts_raw, clip_model, clip_ckpt):
        ...
        if not self.concept_feat_save_dir.exists() or self.force_compute:
            ...
            self.concept_feat = utils.prepare_txt_feat(
                concepts_raw, clip_model_name=clip_model, ckpt_path=None
            )
    ...
    def get_img_n_shot(self, cls2img, n_shots):
        ...
        for cls_name, img_names in cls2img.items():
            ...
            all_img_paths.extend(
                [
                    self.img_root.joinpath("{}{}".format(img_name, self.img_ext))
    ...
    def compute_img_feat(self, cls2img, n_shots, clip_model, clip_ckpt):
        ...
        img_feat = utils.prepare_img_feat(
            all_img_paths, clip_model_name=clip_model, ckpt_path=clip_ckpt
        )
    ...
    def prepare_img_feat(self, splits, n_shots, clip_model, clip_ckpt):
        ...
        self.label = {}
        for mode in ["train", "val", "test"]:
            cls2img, feat_save_dir, label_save_dir = (
                splits[mode],
                self.img_feat_save_dir[mode],
                self.label_save_dir[mode],
            )

            if not feat_save_dir.exists():
                print("compute img feat for {}".format(mode))
                img_feat, label = self.compute_img_feat(
                    cls2img,
                    n_shots if mode == "train" else "all",
                    clip_model,
                    clip_ckpt,
                )
    ...
    def setup(self, stage):
        ...
        self.datasets = {
            mode: ImageFeatDataset(self.img_feat[mode], self.label[mode], self.on_gpu)
            for mode in ["train", "val", "test"]
        }
