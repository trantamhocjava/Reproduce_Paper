...
class AssoConcept(pl.LightningModule):
    ...
    def __init__(self, cfg, init_weight=None, select_idx=None) -> None:
        ...
        concept_feat_path = self.data_root.joinpath(
        ...
        if concept_feat_path.exists():
            if select_idx is None:
                self.select_idx = torch.load(select_idx_path)[: cfg.num_concept]
            else:
            ...
            self.concept2cls = (
                torch.from_numpy(np.load(concept2cls_path)[self.select_idx])
                .long()
                .view(1, -1)
            )
            if os.path.exists(concept_pair_path):
                self.concept2cls1 = (
                    torch.load(concept_pair_path).long()[:, 0].view(1, -1)
                )
                self.concept2cls2 = (
                    torch.load(concept_pair_path).long()[:, 1].view(1, -1)
                )
    ...
    def forward(self, img_feat):
        ...
        cls_feat = (
            mat @ self.concepts
        )  # 𝑔(𝒙, 𝐶) = 𝒙 ⋅ 𝑬_{𝐶}^T, but it's like 𝜎(𝑾)⋅ 𝑬_{𝐶}^T
        sim = (
            img_feat @ cls_feat.t()
        )  # 𝑔(𝒙, 𝐶). 𝜎(𝑾)^T, but it's like 𝒙 ⋅ (𝜎(𝑾)⋅ 𝑬_{𝐶}^T)^T
        return sim
    ...
    def training_step(self, train_batch, batch_idx):
        ...
        if self.cfg.asso_act == "softmax":
            row_l1_norm = torch.linalg.vector_norm(
                F.softmax(self.asso_mat, dim=-1), ord=1, dim=-1
            ).mean()
        # asso_mat sparse regulation
        row_l1_norm = torch.linalg.vector_norm(
            self.asso_mat, ord=1, dim=-1
        ).max()  # .mean()
...
class OurConcept(AssoConcept):
    def __init__(self, cfg, init_weight=None, select_idx=None) -> None:
        ...
        self.attention_block = OurModule(
            dim=self.concepts.shape[-1], num_layers=self.cfg.num_layers
        )
    ...
    def forward(self, image):
        ...
        image_embed = self.attention_block(
            image, residual=self.cfg.use_residual, use_img_norm=self.cfg.use_img_norm
        )
    ...
    def configure_optimizers(self):
        optimizer = torch.optim.SGD(
            self.parameters(), lr=self.cfg.lr, momentum=0.9, weight_decay=1e-4
        )
        ...
        scheduler = torch.optim.lr_scheduler.LinearLR(
            ...
            total_iters=self.cfg.max_epochs,
        )
        return {
            "optimizer": optimizer,
            "lr_scheduler": {
