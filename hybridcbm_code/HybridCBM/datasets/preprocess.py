...
def load_assertions(file_path):
    ...
    with gzip.open(file_path, "rt", encoding="utf-8") as f:
        df = pd.read_csv(
            f,
            sep="\t",
            names=["uri", "relation", "start", "end", "info"],
            usecols=["relation", "start", "end", "info"],
            dtype=str,
        )

    df = df.loc[
        df["start"].str.startswith("/c/en/") & df["end"].str.startswith("/c/en/")
    ]
...
def collect_conceptnet_concepts():
    ...
    raw_data["info"] = raw_data["info"].map(
        lambda x: json.loads(x)["surfaceText"].replace("[", "").replace("]", "")
    )
    concepts = raw_data.loc[raw_data["info"].map(lambda x: len(x.split()) <= 15)][
        "info"
    ].unique()
    os.makedirs(translator_path, exist_ok=True)
    pd.DataFrame({"concept": concepts}).to_json(
        os.path.join(translator_path, "conceptNet.json"), index=False
    )
...
def collect_generated_concepts():
    ...
    concepts.to_json(
        os.path.join(translator_path, "generatedConcepts.json"), index=False
    )
...
def concept(clip_model_type="ViT-L/14", device=4):
    ...
    loader = DataLoader(
        _Dataset(tokens), batch_size=1024, shuffle=False, num_workers=16
    )
    all_embeddings = []
    all_tokens = []
...
def coco(clip_model_type="ViT-L/14", device=4):  # 566747
    ...
    loader = DataLoader(
        _Dataset(data.to_dict(orient="records"), preprocess),
        batch_size=1024,
        shuffle=False,
        num_workers=16,
    )
