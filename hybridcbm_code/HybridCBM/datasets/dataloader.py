...
class DataBank(L.LightningDataModule):
    ...
    def __init__(
        ...
        for mode in ["train", "val", "test"]:
            if mode == "train":
                ...
                self.label_save_dir[mode] = self.img_split_path.joinpath(
                    f"label_{mode}_{n_shots}.pth"
                )
            else:
                self.img_feat_save_dir[mode] = self.img_split_path.joinpath(
            ...
            self.splits[mode]["path"] = self.splits[mode]["path"].map(
                lambda x: str(self.data_root.joinpath(x))
            )
        # ----------------------------------- END -------------------------------------#
        ...
        if self.n_shots != "all":
            self.num_images_per_class = [self.n_shots] * len(
                self.splits["train"]["label"].unique()
            )
        else:
            self.num_images_per_class = [
                len(self.splits["train"][self.splits["train"]["label"] == label])
                for label in self.splits["train"]["label"].unique()
            ]
    ...
    def prepare_img_feature(self):
        ...
        dict_labels = {}
        for mode in ["train", "val", "test"]:
            df, feat_save_dir, label_save_dir = (
                self.splits[mode],
            ...
            if not feat_save_dir.exists():
                ...
                img_features = self.clip_encoder.encode_image(
                    df["path"].tolist(), batch_size=512
                )
                label = torch.tensor(df["label"].tolist())
                torch.save(img_features, feat_save_dir)
                torch.save(label, label_save_dir)
            else:
                img_features, label = torch.load(
                    feat_save_dir, weights_only=True
                ), torch.load(label_save_dir, weights_only=True)
    ...
    def random_sample_img(self, num_images_per_class=1, mode="test"):
        ...
        img_embed_save_path = dir_path.joinpath(
            f"{mode}_{num_images_per_class}_embeddings"
        )
        img_save_dir.mkdir(exist_ok=True, parents=True)
        if not img_embed_save_path.exists():
            df.to_csv(dir_path.joinpath("images.csv"), index=False)
            for img_path in df["path"].tolist():
                img = Image.open(img_path)
                img.save(img_save_dir.joinpath(os.path.basename(img_path)))
            embeddings = self.clip_encoder.encode_image(
                df["path"].tolist(), batch_size=32
            )
            y = torch.LongTensor(df["label"].tolist())
            torch.save({"embeddings": embeddings, "labels": y}, img_embed_save_path)
        else:
            embeddings = torch.load(
                img_embed_save_path, weights_only=True, map_location="cpu"
            )
        return embeddings
    ...
    def setup(self, stage):
        ...
        for mode in ["train", "val", "test"]:
            if self.use_img_features:
                self.datasets[mode] = ImageDataset(
                    self.img_features[mode], self.labels[mode]
                )
            else:
                if mode == "train":
                    self.datasets[mode] = ImageDataset(
                        self.img_features[mode],
                        self.labels[mode],
    ...
    def predict_dataloader(self):
        return DataLoader(
            Dataset_with_name(
                self.datasets["test"], self.splits["test"]["path"].tolist()
            ),
            batch_size=self.batch_size,
            num_workers=self.num_workers,
            pin_memory=self.pin_memory,
