...
from .concept_select import random_select, submodular_select
...
class StaticConceptBank(BaseCB):
    def __init__(
        ...
        self.classes = pd.read_csv(self.data_root.joinpath("class.csv"))[
            "class"
        ].unique()
        ...
        try:
            ...
            print(
                "concept feature not found, please run datamodule precompute_txt first"
            )
    ...
    def classes_embeddings(self):
        return self.clip_encoder.encode_text(
            self.classes, batch_size=128, normalize=False
        )
    ...
    def initialize(self, img_features=None, num_images_per_class=None):
        ...
        if self.num_concept > 0:
            ...
            select_cid = self.select_concept(
                img_features, static_features, num_images_per_class
            )
            self.static_features = torch.nn.Parameter(
                static_features.data[select_cid], requires_grad=False
            )
        ...
        if not self.select_idx_save_path.exists():
            if img_features is None:
                raise FileNotFoundError(
                    "img_features should be provided for concept selection"
                )
            print("select concepts")
            select_cid = self.concept_select_fn(
    ...
    def prepare_text_feature(self, num_images_per_class):
        if not self.concept_feature_save_path.exists():
            if num_images_per_class is None:
                raise FileNotFoundError(
                    "num_images_per_class should be provided for concept selection"
                )
            ...
            concept_features = torch.load(
                self.concept_feature_save_path, weights_only=True
            )
