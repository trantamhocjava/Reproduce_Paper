...
class DynamicConceptBank(BaseCB):
    ...
    def concepts(self):
        ...
        self._concepts = self.translator.decode(
            self.concept_features, entry_length=30, temperature=1, batch_size=32
        )
        self._concepts = pd.DataFrame(
            {"concept": self._concepts, "label": self.concepts_label.tolist()}
