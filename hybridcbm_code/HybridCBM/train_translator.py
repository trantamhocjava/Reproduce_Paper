...
class ConceptDataset(Dataset):
        ...
        if use_embedding:
            ...
            if aug:
                ...
                cache_path = os.path.join(
                    os.path.dirname(data_path), "conceptsBank_tokens.pt"
                )
    ...
    def __getitem__(self, item: int) -> Tuple[torch.Tensor, ...]:
        ...
        if self.aug:
            ...
            text = self.textual_noise_injection(text, p=0.1, error_ratio=0.1)
...
def train_translator(
    ...
    if not checkpoints:
        ...
        print(
            f"Loading checkpoint from {checkpoint_path}, start from epoch {start_epoch}"
        )
    ...
    for epoch in range(epochs):
        ...
        if args.is_master:
            ...
            progress = tqdm(
                total=int(len(train_dataloader) / 10),
                desc=f"{epoch}/{epochs} {output_prefix}",
            )
        else:
            progress = None
        ...
        for idx, data in enumerate(train_dataloader):
            ...
            logits, tokens, loss_token = run_step(
                model, optimizer, scheduler, feature, tokens, loss_ce
            )

            if progress is not None:
                ac = ((logits.argmax(1) == tokens) * (tokens > 0)).sum() / (
                    tokens > 0
                ).sum()
                ...
                if (idx + 1) % 10 == 0:
                    progress.set_postfix(
                        {
                            "loss_token": loss_token_save / 10.0,
                            "acc_token": ac_save / 10.0,
                        }
                    )
                    progress.update()
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--out_dir", default="./weights/translators", help="output directory"
    )
    parser.add_argument("--clip_model", default="ViT-L/14", help="clip model name")
    parser.add_argument(
        "--output_prefix", default="translators", help="prefix for saved filenames"
    )
    ...
    dataset = MergeDataset(
        f"./datasets/ConceptTranslator/ConceptBank_{args.clip_model.replace('/', '_')}.pkl",
