...
def mutual_information_score(
    img_feat, concept_feat, n_shots, num_images_per_class, epsilon=1e-10
):
    num_cls = len(num_images_per_class)
    scores_mean = clip_score(
        img_feat, concept_feat, n_shots, num_images_per_class
    )  # Sim(c,y)
    # 将余弦相似度平移到非负区间
    scores_mean = scores_mean - scores_mean.min()
    normalized_scores = scores_mean / (
        scores_mean.sum(dim=0) * num_cls + epsilon
    )  # Sim_bar(c,y)
    margin_x = normalized_scores.sum(dim=1)  # sum_y in Y Sim_bar(c,y)
    margin_x = margin_x.reshape(-1, 1).repeat(1, num_cls)
    # compute MI and PMI
    pmi = torch.log(
        (normalized_scores + epsilon) / (margin_x * 1 / num_cls + epsilon)
