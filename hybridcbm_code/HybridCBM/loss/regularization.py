...
class L_norm(_Loss):
    ...
    def __init__(self, p=1, loss_weight=1e-4, reduction="max"):
        ...
        logging.info(
            f"setup L{p}-Norm Loss with loss_weight:{loss_weight}, with reduction:{reduction}"
        )
...
class CScoreDiversityLoss(_Loss):
    def __init__(self, loss_weight=1e-4, reduction="mean"):
        ...
        self.reduction = reduction
        logging.info(
            f"setup C-Score Diversity Loss with loss_weight:{loss_weight}, with reduction:{reduction}"
        )
...
class DiscriminabilityLoss(_Loss):
    ...
    def __init__(
        self, loss_weight, num_classes=4, alpha=1, beta=0.1, relative_margin=0.85
    ):
    ...
    def compute(self, image, concept_vectors, target, classes_embeddings=None):
        ...
        concept_vectors = concept_vectors.reshape(
            (self.num_classes, -1, concept_vectors.shape[-1])
        )
        ...
        if classes_embeddings is not None and self.alpha > 0:
            ...
            if classes_similarity.shape[0] != classes_similarity.shape[1]:
                raise ValueError(
                    f"Expected {self.num_classes} classes, but got {classes_similarity.shape}"
                )
