...
class TrainHelper:
    ...
    def load_checkpoint(self, use_last_ckpt=False):
        ...
        if (
            "use_last_ckpt" in self.config and self.config["use_last_ckpt"] is not None
        ) or use_last_ckpt:
    ...
    def test(self, log_file="test.log"):
        ...
        model.eval()
        logger.info(
            f"conceptBank requires_grad: {model.conceptbank.dynamic_bank.concept_features.requires_grad}"
        )
        # 创建Trainer
        trainer = Trainer(
    ...
    def train(self):
        ...
        trainer.fit(
            ...
            ckpt_path=self.load_checkpoint(use_last_ckpt=True),
        )
    ...
    def run(self):
        if self.config.multi and self.runner is not None:
            ...
            if not self.config.test and not os.path.exists(
                os.path.join(self.config.exp_root, "test.log")
            ):
                ...
                try:
                    ...
                    print(
                        f"config file not found in {self.config.exp_root}, Generate a new one"
                    )
                    Config.save_config(self.config, force=False)

                self.test()
            elif not os.path.exists(
                os.path.join(self.config.exp_root, "clip_metrics.csv")
            ):
                self.test()
